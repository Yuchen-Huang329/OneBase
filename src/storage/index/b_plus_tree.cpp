...
auto BPLUSTREE_TYPE::IsEmpty() const -> bool {
  ...
  while (!tree_page->IsLeafPage()) {
    ...
    // Degenerate internal root / subtree with no valid entries.
  ...
  // Scan the leaf chain. If any leaf still has records, tree is not empty.
...
void BPLUSTREE_TYPE::Remove(const KeyType &key) {
  ...
  if (leaf->IsRootPage()) {
    if (new_size == 0) {
      page_id_t old_root = leaf_page->GetPageId();
  ...
  coalesce_or_redistribute = [&](Page *page) {
    ...
    if (tree_page->IsRootPage()) {
      // Root leaf: empty tree
      if (tree_page->IsLeafPage()) {
        if (tree_page->GetSize() == 0) {
          page_id_t old_root = page->GetPageId();
          root_page_id_ = INVALID_PAGE_ID;
          bpm_->UnpinPage(old_root, true);
          bpm_->DeletePage(old_root);
        } else {
      ...
      // Root internal: keep shrinking while only one child remains
      if (tree_page->GetSize() == 1) {
        ...
        coalesce_or_redistribute(child_page);
        return;
      }
  ...
  // Final normalization of root after deletion
  while (root_page_id_ != INVALID_PAGE_ID) {
    Page *root_page = bpm_->FetchPage(root_page_id_);
    auto *root_tree = reinterpret_cast<BPlusTreePage *>(root_page->GetData());

    if (root_tree->IsLeafPage()) {
      if (root_tree->GetSize() == 0) {
    ...
    if (root_tree->GetSize() == 1) {
      auto *root_internal = reinterpret_cast<InternalPage *>(root_tree);
